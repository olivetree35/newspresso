...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("\n[HRI 스크래퍼] 수집 기간을 입력하세요 (형식: YYYY-MM-DD)")
        s_input = input("▶ 시작일 (Enter=2025-12-01): ").strip()
        e_input = input("▶ 종료일 (Enter=2026-01-31): ").strip()
        
        start_date = s_input if s_input else "2025-12-01"
        end_date = e_input if e_input else "2026-01-31"
        print(f"👉 설정된 기간: {start_date} ~ {end_date}\n")
    else:
